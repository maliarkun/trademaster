...
def get_historical_prices(base, quote, timeframe='day', limit=500):
    ...
    if data.get('Response') == 'Success' and 'Data' in data and 'Data' in data['Data']:
        ...
        prices = [{'high': float(entry['high']), 'low': float(entry['low']), 'close': float(entry['close']),
                   'time': entry['time']} for entry in entries]
...
def calculate_stochastic(prices, period=14, smooth_k=3, smooth_d=3):
    ...
    d_smooth_values = [sum(k_smooth_values[i - smooth_d + 1:i + 1]) / smooth_d for i in
                       range(smooth_d - 1, len(k_smooth_values))]
...
def calculate_ichimoku(prices, tenkan_period=9, kijun_period=26, senkou_b_period=52):
    ...
    senkou_span_b = (max(p['high'] for p in prices[-senkou_b_period:]) + min(
        p['low'] for p in prices[-senkou_b_period:])) / 2
...
def calculate_atr(prices, period=14):
    ...
    if not prices or len(prices) < period + 1:
        return None
    tr_list = [max(prices[i]['high'] - prices[i]['low'], abs(prices[i]['high'] - prices[i - 1]['close']),
                   abs(prices[i]['low'] - prices[i - 1]['close'])) for i in range(1, len(prices))]
...
def trading_pairs(request):
    ...
    try:
        ...
        for pair in pairs:
            ...
            current_price = float(
                historical_prices[-1]['close']) if current_price == 'Bilinmeyen' and historical_prices else float(
                current_price) if current_price != 'Bilinmeyen' else 'Veri Yok'

            # Ichimoku Hesaplamaları ve Sinyalleri
            ichimoku = calculate_ichimoku(historical_prices)
            if ichimoku:
                ichimoku_signal = "Yükseliş Trendi" if current_price > ichimoku[
                    'kijun_sen'] else "Düşüş Trendi" if current_price < ichimoku['kijun_sen'] else "Nötr"
                if len(historical_prices) >= 2:
                    prev_ichimoku = calculate_ichimoku(historical_prices[:-1])
                    if prev_ichimoku and prev_ichimoku['tenkan_sen'] < prev_ichimoku['kijun_sen'] and ichimoku[
                        'tenkan_sen'] > ichimoku['kijun_sen']:
                        ichimoku_signal += " (Alım Sinyali)"
                    elif prev_ichimoku and prev_ichimoku['tenkan_sen'] > prev_ichimoku['kijun_sen'] and ichimoku[
                        'tenkan_sen'] < ichimoku['kijun_sen']:
                        ichimoku_signal += " (Satış Sinyali)"
                ichimoku_signal += " (Bulut Üstünde)" if current_price > max(ichimoku['senkou_span_a'], ichimoku[
                    'senkou_span_b']) else " (Bulut Altında)" if current_price < min(ichimoku['senkou_span_a'],
                                                                                     ichimoku[
                                                                                         'senkou_span_b']) else " (Bulut İçinde)"
            else:
            ...
            if historical_prices:
                ...
                closest_level = min(fibonacci_levels_raw, key=lambda level: abs(
                    fibonacci_levels_raw[level] - current_price) if current_price != 'Veri Yok' else float('inf'))
                distance = abs(
                    fibonacci_levels_raw[closest_level] - current_price) if current_price != 'Veri Yok' else float(
                    'inf')
            ...
            if rise_probability != 'Veri Yok':
                ...
                if not hasattr(pair, 'last_signal') or not hasattr(pair, 'notification_threshold'):
                    ...
                    if current_signal == "yükseliş" and rise_probability >= getattr(pair, 'notification_threshold', 50):
                        message = f"{key} için Yükselme İhtimali: %{rise_probability} (Güncel Fiyat: {current_price}) - [test]"
                        send_telegram_message(token, chat_id, message)
                        pair.last_signal = "yükseliş"
                        pair.save()
                    elif current_signal == "düşüş" and fall_probability >= getattr(pair, 'notification_threshold', 50):
                        message = f"{key} için Düşüş İhtimali: %{fall_probability} (Güncel Fiyat: {current_price}) - [test]"
                        send_telegram_message(token, chat_id, message)
...
def pair_detail(request, pair):
    ...
    try:
        ...
        current_price = get_current_prices([TradingPair(base_currency=base, quote_currency=quote)]).get(pair,
                                                                                                        'Bilinmeyen')
        current_price = float(
            historical_prices[-1]['close']) if current_price == 'Bilinmeyen' and historical_prices else float(
            current_price) if current_price != 'Bilinmeyen' else 'Veri Yok'
        ...
        if ichimoku:
            ...
            if len(historical_prices) >= 2:
                ...
                if prev_ichimoku and prev_ichimoku['tenkan_sen'] < prev_ichimoku['kijun_sen'] and ichimoku[
                    ...
                    ichimoku_signal += " (Satış Sinyali)"
            ichimoku_signal += " (Bulut Üstünde)" if current_price > max(ichimoku['senkou_span_a'], ichimoku[
                'senkou_span_b']) else " (Bulut Altında)" if current_price < min(ichimoku['senkou_span_a'], ichimoku[
                'senkou_span_b']) else " (Bulut İçinde)"

            # Ichimoku Yüzdelik Tahmini Hesaplama
            score = 0
            ...
            if current_price > ichimoku['senkou_span_a'] and current_price > ichimoku['senkou_span_b']:
                score += 1
            if ichimoku['chikou_span'] and ichimoku['chikou_span'] > current_price:
                score += 1
            if current_price < ichimoku['tenkan_sen']:
                score -= 1
            if current_price < ichimoku['kijun_sen']:
            ...
            if ichimoku['chikou_span'] and ichimoku['chikou_span'] < current_price:
                score -= 1
            ichimoku_percentage = (score + 5) / 10 * 100  # -5 ile +5 aralığını 0-100 aralığına çeviriyoruz
            ichimoku_fall_percentage = 100 - ichimoku_percentage  # Düşüş ihtimalini hesaplıyoruz
        else:
            ichimoku_signal = 'Veri Yok'
            ichimoku_percentage = 50  # Veri yoksa nötr bir tahmin
            ichimoku_fall_percentage = 50  # Veri yoksa nötr bir tahmin
        ...
        price_change_7d = round(
            ((historical_prices[-1]['close'] - historical_prices[-7]['close']) / historical_prices[-7]['close']) * 100,
            2) if len(historical_prices) >= 7 else 'Veri Yok'
        ...
        fibonacci_levels = {
            level: {'value': round(value, 2), 'turn_probability': 30, 'rise_probability': 35, 'fall_probability': 35}
        ...
        context = {
            ...
            'ichimoku': ichimoku if ichimoku else 'Veri Yok',
            'ichimoku_signal': ichimoku_signal,
            'ichimoku_percentage': ichimoku_percentage,  # Yükseliş ihtimali
            'ichimoku_fall_percentage': ichimoku_fall_percentage,  # Düşüş ihtimali
            'atr': atr if atr else 'Veri Yok',
            'vwap': vwap if vwap else 'Veri Yok'
        }
